<template>
  <div class="p-4 lg:p-8">
    <Listbox as="div" v-model="selected">
      <ListboxLabel class="block text-sm font-medium text-gray-700 dark:text-gray-500">Jamoa a'zolari</ListboxLabel>
      <div class="relative mt-1">
        <ListboxButton
          ...
          <span class="flex items-center">
            <img :src="selected.league.iconUrls.small || selected.league.iconUrls.tiny" :alt="selected.league.name"
        ...
        <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100"
          ...
          <ListboxOptions
            ...
            <ListboxOption as="template" v-for="member in members" :key="member.tag" :value="member"
              v-slot="{ active, selected }">
              <li
                :class="[active ? 'text-white bg-indigo-600' : 'text-gray-900 dark:text-white', 'relative cursor-default select-none py-2 pl-3 pr-9']">
                <div class="flex items-center">
                  <img :src="member.league.iconUrls.small || member.league.iconUrls.tiny" :alt="member.clanRank"
                    class="h-6 w-6 flex-shrink-0" />
                  <span class="ml-3 block truncate tracking-wide font-clash">
                    {{ member.name }}
                  </span>
